...
本地 Comfy H3 的 R2V 可同时连接可选首帧与人物、视频、声音参考。首帧通过 `MiniMaxH3AddGuide(frame_idx=0)` 提供目标时间位置上的图像引导，参考素材仍保留原槽位；不将声音参考当作时间轴上的固定音轨。它不是像素级首帧复制，分辨率适配和 VAE 编解码也可能改变画面，仍须检查实际接缝。上游真实尾帧连线继续保留 `source_run_id` 与 `require_accept`，不要通过普通上传绕过采用来源。
