# Handoff 对照表：短剧剧本 → 项目桥接包

遵守[项目共享生产规则](../../../../docs/ai-system-prompt.md)。本文件只映射已写剧本的创作事实，具体视频交接由下游 Skill 负责。

本文件供 `/桥接` 使用。目标是把 `episodes/epNNN.md` 的可拍事实整理成创作侧简报，交给 **zero-to-story** 继续完成故事板、视觉资产、Panel、H3 提示词和 Canvas 交接。短剧编剧 Skill 不调用 Canvas 或提供方，也不决定视频模式。

---

## 1. 产物分层

| 层 | 路径 | 消费者 | 是否改写 ep 剧本 |
|----|------|--------|------------------|
| 编剧主产物 | `episodes/epNNN.md` | 人类审剧 / 导出 | 否 |
| 桥接简报 | `handoff/epNNN/storyboard_brief.md` | zero-to-story | 否（只读 ep） |
| 可拍角色卡 | `handoff/characters_visual.md` | zero-to-story / 资产阶段 | 否（抽取事实） |
| 剪辑说明 | `handoff/epNNN/cut_notes.md` | zero-to-story / 用户 | 否（补充取舍） |

---

## 2. 字段对照

| 短剧来源 | handoff 落点 | 规则 |
|----------|--------------|------|
| 集标题 + logline 切片 | `storyboard_brief.md` → 故事梗概 | 2–3 句，只含本集可拍主线 |
| `characters.md` 叙事档案 | `characters_visual.md` + brief 角色列表 | 只留外貌/服饰/标志特征/关键道具；动机弧光可附一行「表演提示」但非必填 |
| 场次标题（地点·时间·内外） | brief「场景列表」 | 保留用户指定的场景；只有已授权压缩时，常规连载集才参考 3–6 场压成 **1–2** 个场景，并记录取舍 |
| △ 镜头描写 | brief「候选 Panel/节拍」画面描述 | 每条补全前景/中/远、左/右/中、主体关系和可见结束状态 |
| 景别中文（全景/中景/近景/特写…） | 候选 Panel 的创作事实 | 保持原意；Camera Setup 和 operation 由 zero-to-story 按当前关系确定 |
| 角色对白（15–25 句） | 候选 Panel 的对白栏或 `cut_notes.md` | 按可拍节拍组织，不机械一行对白配一个镜头；用户要求逐字保留时完整交接，已授权精简时才提炼关键句 |
| ♪ 音乐提示 | `cut_notes.md` 音频意图 | 区分对白、环境声、动作声、外部口播和临时音乐；不把意图写成已存在的音频文件 |
| 🎣 钩子 / 下集预告 | **仅** `cut_notes.md` | 不伪装成模型素材或最终字幕，除非用户另行确认字幕内容 |
| 💰 付费墙信息 | cut_notes | 作为叙事节奏提示，不进入 Canvas 节点的隐藏控制字段 |
| 情绪强度 / 关键词 | brief 项目信息和节奏意图 | 与本集一致，不生成分数或运行时 QC 等级 |
| 全剧类型/基调 | `handoff/project.json` | 剧级默认值 |

---

## 3. 时长与 Panel 化

桥接可保留用户希望的本集总时长（常见 15–60 秒）和 9:16 画幅，但不要把整集当作一个生成 Clip，也不要用固定镜头数替代 Panel 计划。

- zero-to-story 按场景因果和 Camera Setup 将本集拆成多个 Panel；每个 Panel 目标时长为 4–15 秒，并对应一个 Clip。
- 相邻 Panel 的连续关系、真实首尾帧和 operation 在 zero-to-story 的故事板/蓝图中确认；桥接只提供可见事实和意图。
- 本集可拍节拍数量不是视频请求数量；同一 Panel 内多个 Beat 可以属于同一个 Camera Setup。

---

## 4. 未指定制作预算时的建议

这些是常见默认，不能覆盖用户指定的角色、场景、总时长或画幅。90 秒或更多场景的单集按原范围交接，由下游拆分多个合法 Panel；不擅自压到示例预算。JSON 模板中的时长和两条场景占位也只是例子，按实际内容填写。

| 字段 | 短剧默认 |
|------|----------|
| `aspect_ratio` | `9:16` |
| `delivery_width` / `height` | 1080 / 1920 |
| 角色 | 未指定时建议 2–3 个主要出镜角色；超出时保留角色并在 `cut_notes.md` 说明调度要求，删减须属于用户授权范围 |
| 场景 | 未指定时建议 1–2 个主要可拍场景；保留用户指定场景，同地点场次仅在不丢剧情/时间信息时合并 |
| 节奏 | 按基调给出 `fast` / `medium` / `slow` 的文字建议，不写模型或运行时参数 |
| 音频 | 写清对白、环境声和外部口播的意图；实际音频文件由下游确认，并在已接受 Canvas 输出的确定性后期中处理 |
| 字幕 | 只有用户明确要求时才准备字幕内容；画面中的标题、按钮和动效文字不等于字幕 |

---

## 5. 场次折叠示例

源剧本 4 场：走廊危机 / 高档大堂 / 骑行接单 / 出租屋。

以下仅示范已经授权压缩时的取舍；若用户要求保留原场景，就逐场交接：

1. **场景 A — 夜城跑腿外景+大堂**（社死冲突 + 接单钩子）
2. **场景 B — 老旧公寓走廊**（开场闪回危机，可作片头或片尾镜组）

出租屋若仅作情绪缓冲，可并入场景 A 的「室内中转」2–3 镜，或删镜以保时长。

---

## 6. 角色叙事 → 视觉卡

| 保留 | 丢弃或降级到 cut_notes |
|------|------------------------|
| 年龄外观、发型、体型 | 核心动机长文 |
| 服饰（骑手头盔雨衣、睡袍、相机包） | 人物弧光全文 |
| 标志动作/道具（歪戴头盔、双机位包、两碗面） | 秘密剧透（可一句「表演暗示」） |
| 一句辨识台词（可选） | 反派四阶关系网 |

---

## 7. 交给 zero-to-story

默认保留桥接源文件：

```
.short-drama/{drama_title}/handoff/ep001/storyboard_brief.md
.short-drama/{drama_title}/handoff/characters_visual.md
.short-drama/{drama_title}/handoff/ep001/cut_notes.md
```

用户明确开始视频项目，或当前指令已经授权视频交接后，由 zero-to-story 或调用方把已确定的 brief、视觉素材和最新创作文件放入：

```
workspace/projects/<project_id>/
```

随后按 zero-to-story 的 Panel 流程：创作方案确定（复用当前指令或已有确认）→ 单 Panel 交给 `h3-prompt-writing` → 保存 Canvas 视频节点 → 页面确认或对话明确授权后冻结快照并串行执行 → 实际媒体回填与审片 → `ACCEPT` 后按需提取真实尾帧 → 全部 Panel 接受后对已接受输出做确定性成片处理。短剧编剧 Skill 不复制、重命名或删除用户媒体，也不维护执行状态。
