...
2026-09-07：评测口径同步当前 scope 和无源引用合同；`bounded` 只按授权删除纯空话整句，`in-place` 保留整句并采用 audit-only 提示；普通改写不因删除清单等待确认。历史结果与原始样本不随本说明改写。
...
- 按规则处理原文：默认输出改写后的文本；无源论断按模式和 scope 处理：`rewrite-safe` 下，数字、预测或结论若完全依赖无源引用且不能独立成立，可以整条删除；`audit-only` 对该论断输出缺来源 / 缺归属的风险说明，不强行改写或删除它；`rewrite-with-placeholder` 只保留待补来源占位。`audit-only` 只约束无源论断本身，同段其他病灶仍按各自规则清理，不能整段只输出风险说明
...
- `Long-form / in-place` 样本不删整句、不合并相邻句、不重排段落；二次 Residual Audit 也只能做句内改写或风险提示。字数留存率目标 ≥ 0.90，硬下限 0.85
- `Bounded` 样本可在用户已授权改写时直接删除确属纯空话的整句；不把实句或承担节奏的重复放进删除清单，不合并相邻事实句，不把商业黑话壳句和紧随其后的数据句合并成一句。删除理由按需附上，不等待确认
