...
2026-09-07：统一 scope 的实际动作与入口合同。用户已授权改写时，`bounded` 直接删除仅由空话组成的整句；删除理由按审计需要附上，不默认列清单或等待确认。保留原始具体事实、数字关系、节奏和 `in-place` 的整句边界；无源数字、预测或结论只有在 `rewrite-safe` 且完全依赖无源引用时才可整条删除，`audit-only` 保留并标出缺来源。
...
- `bounded`：不并句、不重排、不删实句和承担节奏的重复；用户授权改写时，遇到确实仅由空话组成的整句（总结式收尾、价值拔高骨架、无独立信息的权威铺垫、整句旁白）直接删除。需要审计或复核时再列原句和“删除后信息不变”的理由，不等待用户确认。原始具体事实和承担节奏的实句仍保留；歧义数量关系不猜改；无源论断按所选模式处理：`rewrite-safe` 下，数字、预测或结论若完全依赖无源引用且去掉无源引用后不能独立成立，可整条删除；`audit-only` 保留并标出缺来源；`rewrite-with-placeholder` 只保留明确的待补来源占位。保留无源论断时不能改数值或跨度；句首可剥离的引导词仍按各节的 `in-place 替代动作` 句内清理
- `in-place`：整句一律不删（空话也不删），只做各节的 `in-place 替代动作`；遇到整句空话保留原句，只有用户要求标注或审计确有必要时才提示风险，不把提示变成额外确认关卡

判断"整句空话 vs 句首引导词"：删掉句首提示词后，剩余部分若仍是带信息的可读句 → 句内洗；若什么都不剩、或只剩另一句空话 → 用户授权的 `bounded` 直接删除，或在 `in-place` 保留原句。若句子仍含具体事实或承担节奏，不把它判为空话；歧义数量关系不猜改。无源论断按上面的模式与 scope 规则处理：`rewrite-safe` 下，数字、预测或结论若完全依赖无源引用且不能独立成立，`bounded` 才可整条删除；`audit-only` 保留并标出缺来源；`in-place` 无论模式都保留整句。保留无源论断时不能改数值或跨度。各节写到“保留原句并标注”的内容只适用于 `in-place` 的按需风险提示，不构成默认确认流程。

校准删除范围用**压缩试验**：假设把全文删掉三分之一，如果事实、动作、判断和阅读体验几乎没变，原文就是在注水，可以多删几处确属纯空话的整句；如果一删就掉信息，说明原文密度本来就够，删除动作应该很少甚至没有。这个试验只用来判断"整篇注水到什么程度"，不替代上面三条删除判据——每一条仍然要单独满足"删了不丢信息"。
...
- 如果整句只剩空总结，不要为了凑字数硬补新内容；`in-place` 下保留原句，只有用户要求标注或审计有必要时才提示 `[空总结，建议评估是否删除]`
- 不把前后句合并成一个更短总结
...
- 不删整句，先拆掉句内拔高骨架：`这不仅仅是 X，更是 Y` 只有在 `X` 只是姿态或重复铺垫、没有独立事实、判断或动作时才可改成 `这是 Y`；如果 `X` 承载独立信息，必须保留 `X` 和 `Y` 的信息，在同一句内改为直接陈述，不删前半句或改变句子数量
- `真正的 X 不是 A，而是 B` 只有在 `A` 只是被虚构的误解或姿态对照、没有独立事实时才可改成 `X 更看 B` 或 `B 更能影响 X`；若 `A` 和 `B` 都承载信息，保留两边并在同一句内改成中性连接，不靠删除前半句制造洞见感
...
- 第二遍只做轻量修正：
  - `structural` 下可删掉一个残留开场或空总结、合并两句过匀的事实句，或拆开一处过满的句子
  - `bounded` 下只能沿用已授权的纯空话删除边界；实句不合并、不重排，残留可通过句内改写处理
  - `in-place` 下保留所有整句，只把残留 narrator / 空泛判断压回直接句，不合并、不拆句、不重排；整句空话按合同提示风险而不删除
  - 同型句式骨架超出密度阈值时，只改超出的那几处，换成中性连接或直接陈述；`bounded` / `in-place` 仍不得突破各自 scope
- `docs / status / code-context` 默认更保守：只有残留真的明显，而且不影响事实、术语和正式语体时才动
- `bounded` 和 `in-place` 的第二遍不得合并、重排或把实句改成摘要；`bounded` 只能沿用已授权的纯空话删除边界，`in-place` 连整句空话也保留，只做句内替换或按合同提示风险。若第二遍需要大改结构才能“更像人”，那通常说明该停在第一遍，而不是继续抛
...
- 装饰细节常常占满整句，`in-place` 下不删句；只有用户要求标注或审计有必要时才提示 `[无来源细节，建议评估是否删除]`
- 细节只是句内修饰成分时（`他盯着那行报错，窗外下着雨`），可以句内删掉修饰，保留主干
- `bounded` 下，整句装饰细节若确实无来源且删后信息不变，可直接删除；需要追溯时再记录理由“无来源且删后信息不变”。用户提供的经历、复现条件或承担因果的细节不能按此删除
