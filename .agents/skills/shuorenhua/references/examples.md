...
2026-09-07：同步当前 scope、无源引用和授权口径；`bounded` 示例直接交付已授权成稿，删除理由仅在审计有用时附上，不把删除清单变成默认确认步骤。`in-place` 示例保留整句，只做句内处理。
...
> bounded 的输出以句内洗过的正文为主；已授权改写可直接删除纯空话整句，需要审计时再附删除理由，不等待确认。示例（合成文本）：
