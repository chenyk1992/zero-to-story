...
description: |
  ...
  调用 /v1/voice_clone 生成可用于后续 TTS 的 voice_id；包含授权检查与复用、
...
用户已交付音频并要求处理时，可先通过 `ffprobe` 做本地只读检查，必要时在系统临时目录准备文件；这些动作不等于取得克隆授权。上传或调用 `/v1/voice_clone` 前，确认当前指令或已有记录明确说明：操作者拥有该音频的合法使用权、说话人已授权音色克隆、用途合法。已有针对同一音频/说话人的有效说明直接复用，不重复提问。若授权身份或用途不清，只提出一个简短的定向问题；不要把固定问卷当作每次任务的必经格式，也不要从笼统的“克隆这个音频”推断授权。

```yaml
question: "请确认：你拥有该音频的合法使用权，并已取得说话人进行音色克隆的授权，且会将结果用于合法用途。"
options:
  - label: "确认并继续"
...
用户明确取消整个任务时停止全部处理；拒绝授权或表达不确定时停止上传和克隆，在授权未清楚前不得发起真实 API 请求，但可以保留已完成的本地只读检查，不把它当作授权判断。

## 2. 本地前置检查

用户已交付音频并请求处理后即可在本地检查：
...
在发起真实请求前复用用户已给参数并采用以下安全默认值；只有缺少的参数会改变付费请求、音色身份或处理结果时才询问：

- `voice_id`：用户提供时验证其长度为 8–256、首字符为英文字母、只含字母/数字/`-`/`_`，且末字符不能为 `-` 或 `_`；用户未提供时由脚本生成唯一 ID。
- 试听文本：只有用户明确要求试听且提供/确认文本时才通过 `--text` 传入，最多 1000 字符；否则使用 `--no-demo`，避免未授权的 TTS 计费。
- 模型：默认 `speech-2.8-hd`。
...
修改技能或排查配置时，在当前请求覆盖该音频的本地检查时使用 `--dry-run`，它只做本地音频校验和请求计划输出，不上传文件：
...
- 不得跳过或代替用户完成授权确认；清楚且仍适用的既有授权可复用，不要重复索要。
