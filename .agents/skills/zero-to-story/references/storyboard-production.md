...
共同的角色、Panel ready、实际验收和交接规则见[项目共享生产规则](../../../../docs/ai-system-prompt.md)。本文只规定故事板、Camera Setup、六 Beat、operation 和视觉计划。
...
主会话兼任章节导演，先决定观众在这一章应理解和感受什么，再选择可实现的拍法。导演统一维护叙事视点、表演动机、空间调度、视觉语言、节奏和最终呈现；不把运镜数量或“电影感”当作质量标准。具体取舍写入故事板，不另建导演报告或新的审批节点。
...
- 需要分镜板或多张普通素材分别承载身份、场景/构图控制，或用户明确选择时用 R2V；普通参考采用 `reference_assets` 策略（见[蓝图](creative-blueprint.md)），不强制分镜板，不声称精确首帧锁。
...
4. **生成必要视觉控制资产。** 按 `generation.panel_plans` 只物化当前 Panel 运行时必要资产：`none`、`storyboard_board`、`reference_assets`、`scene_keyframe` 或 `last_frame`。先满足当前 Panel 的真实依赖，再按当前工具额度有限并行互不引用的资产；同一分镜板始终一次生成。已有可用资产复用，`planning_only` 默认不生成。当前 Panel 的自身事实、提示词、必要资产、参数、音频/后期责任和授权齐全即可 ready，不等待独立 Panel 的资产。
