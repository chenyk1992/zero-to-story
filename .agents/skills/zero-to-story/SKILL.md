...
1. **设计故事和镜头。** 先决定观众此刻应理解什么、人物为何行动，再安排走位、景别、切镜和声音。连续场景按动作与声音的自然完成点拆 Panel，保护完整对白、停顿与句尾，并规划环境声、BGM 的跨段关系；不按时长上限机械切段。具体依据见[连续场景的分段](references/storyboard-production.md#连续场景的分段)，不另写评分表。
...
- 连续场景跨 Panel 时，区分同机位续演、换机位与换场；静态角色/场景参考不等于继承实际末态。不得为提前生成而取消真实依赖。相邻片段按[画面与声音接缝](references/video-qc.md#画面与声音接缝)一起检查，单片通过不代表接缝成立。
