...
def make_storyboard(num_shots: int = 1, project_id: str = "proj-preview") -> Storyboard:
    """Create a storyboard with N shots."""
    shots = []
    for i in range(num_shots):
        shot = Shot(
            ...
            camera=Camera(shot_size="medium", movement="static", angle="eye_level"),
    ...
    return Storyboard(
        project=ProjectInfo(project_id=project_id, title="Preview Test"),
        shots=shots,
        style=StyleGuide(
...
class TestBuildRequests:
    ...
    def test_single_shot_single_sheet(self, service):
        ...
        assert batch.requests[0].shot_range == ["shot_001"]

    def test_8_shots_single_sheet(self, service):
        """8 shots = exactly one sheet (no bridge)."""
    ...
    def test_9_shots_two_sheets(self, service):
        """9 shots = 2 sheets, second sheet bridges from shot 8."""
        storyboard = make_storyboard(9)
        batch = service.build_requests(storyboard, "proj-preview")
        assert len(batch.requests) == 2
        # Sheet 1: shots 1-8
        assert batch.requests[0].shot_range == [f"shot_{i:03d}" for i in range(1, 9)]
        # Sheet 2: shots 8-9 (panel 1 = shot 8, bridge)
        assert batch.requests[1].shot_range == ["shot_008", "shot_009"]

    def test_15_shots_two_sheets(self, service):
        """15 shots = 2 sheets, second sheet: shots 8-15."""
        storyboard = make_storyboard(15)
        batch = service.build_requests(storyboard, "proj-preview")
        assert len(batch.requests) == 2
        assert len(batch.requests[0].shot_range) == 8
        assert len(batch.requests[1].shot_range) == 8
        # Panel 1 of sheet 2 should be shot 8
        assert batch.requests[1].shot_range[0] == "shot_008"

    def test_16_shots_three_sheets(self, service):
        """16 shots = 3 sheets with overlap chaining."""
        storyboard = make_storyboard(16)
        batch = service.build_requests(storyboard, "proj-preview")
        assert len(batch.requests) == 3
        # Sheet 1: shots 1-8
        assert batch.requests[0].shot_range[0] == "shot_001"
        # Sheet 2: shots 8-15 (bridge = shot 8)
        assert batch.requests[1].shot_range[0] == "shot_008"
        # Sheet 3: shots 15-16 (bridge = shot 15)
        assert batch.requests[2].shot_range[0] == "shot_015"

    def test_22_shots_three_sheets(self, service):
        """22 shots = 3 sheets: 1-8, 8-15, 15-22."""
    ...
    def test_prompt_contains_shot_descriptions(self, service):
        """Prompt should include shot descriptions and panel layout."""
    ...
    def test_prompt_contains_medium_lock(self, service):
        """Prompt should include medium lock from style."""
    ...
    def test_prompt_contains_style_keywords(self, service):
        """Prompt should include style keywords from style."""
        storyboard = make_storyboard(2)
        batch = service.build_requests(storyboard, "proj-preview")
        prompt = batch.requests[0].prompt
    ...
    def test_bridge_panel_in_prompt(self, service):
        """Second sheet prompt should mention bridge panel."""
        storyboard = make_storyboard(9)
        batch = service.build_requests(storyboard, "proj-preview")
    ...
    def test_panel_descriptions_include_key_props(self, service):
        """Panel descriptions should include character key props for identity anchoring."""
        from lfo.storyboard.storyboard import Character, CharacterAppearance
        chenmo = Character(
        ...
        # Add Chenmo to each shot
        for shot in storyboard.shots:
            shot.characters = [
                CharacterAppearance(character_id="char_chenmo", screen_position="center"),
            ]
        batch = service.build_requests(storyboard, "proj-preview")
        prompt = batch.requests[0].prompt
    ...
    def test_template_has_hard_no_text_constraint(self, service):
        """Preview template must enforce NO TEXT as a hard constraint."""
        storyboard = make_storyboard(2)
        batch = service.build_requests(storyboard, "proj-preview")
        prompt = batch.requests[0].prompt
        # Should have multiple "NO" negations (panel-level + hard constraint block)
    ...
    def test_output_paths(self, service):
        """Each request should have a unique output path."""
        storyboard = make_storyboard(16)
        batch = service.build_requests(storyboard, "proj-preview")
    ...
    def test_batch_metadata(self, service):
        """Batch should have correct metadata."""
        storyboard = make_storyboard(3)
        batch = service.build_requests(storyboard, "proj-preview")
...
class TestWriteRequests:
    """Test writing preview batch to disk."""
...
class TestCollectResults:
    """Test preview result collection."""

    def _make_batch_and_results(self, service, storyboard, tmp_path):
        """Helper: build batch, write it, simulate agent results."""
        batch = service.build_requests(storyboard, "proj-preview")
    ...
    def test_collect_missing_file(self, service, tmp_path):
        ...
        # Overwrite results with non-existent file path
    ...
    def test_asset_registered_in_db(self, service, tmp_path):
        ...
        assert row[1] == "image"
        assert row[2] is None  # task_id is NULL for preview sheets
