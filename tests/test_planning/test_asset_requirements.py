...
class TestPlanPanelAssetRequirements:
    ...
    def test_r2v_character_and_composition_refs(self):
        panel = Panel(panel_id="panel_001")
        pack = build_panel_pack(
            panel_id="panel_001",
            beat_range=(1, 8),
    ...
    def test_r2v_with_bw_no_composition_requirement(self):
        panel = Panel(panel_id="panel_001")
        pack = build_panel_pack(
            panel_id="panel_001",
            beat_range=(1, 8),
        ...
        reqs = plan_panel_asset_requirements(panel, pack, sel)
        roles = [r.asset_role for r in reqs]
        assert "composition_ref" not in roles
        assert roles.count("character_ref") == 1
