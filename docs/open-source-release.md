...
`workspace/` 保存本机工作流验证产物，不纳入 Git。历史测试目录 `.short-drama/` 和运行数据库也由忽略规则排除。已跟踪文件需使用 `git rm --cached` 取消跟踪，本机文件可继续使用；不需要另建仓库或导出源码。

提交前检查：

```powershell
git ls-files -- workspace .short-drama db
