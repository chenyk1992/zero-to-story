...
视频接受必须基于实际可播放输出。记录位置、姿态、手持物、接触关系、动作是否完成等实际末态，以及声音、对白、停顿、同步和可懂度等实际音频证据；证据不足就保留未知或阻塞。画布是唯一生产入口，具体边界见 [AGENTS.md](../AGENTS.md)、共享生产规则和[画布指南](canvas-guide.md)，不在各 Skill 中重复添加审批门槛。
